...
return (function()
	...
	local function makeFrame(obj)
		...
		linesFrame.MouseEnter:Connect(function()
			service.UserInputService.MouseIcon = "rbxasset://textures/Cursors/KeyboardMouse/IBeamCursor.png"
		end)
		
		linesFrame.MouseLeave:Connect(function()
			service.UserInputService.MouseIcon = ""
		end)

		lineTweens.Invis = tweenService:Create(cursor,TweenInfo.new(0.2,Enum.EasingStyle.Quart,Enum.EasingDirection.Out),{BackgroundTransparency = 1})
		...
		service.UserInputService.InputBegan:Connect(function(input)
			if input.UserInputType == Enum.UserInputType.MouseButton1 and not Lib.CheckMouseInGui(linesFrame) then
				obj:CursorAnim(false)
			end
		end)
	...
	funcs.CursorAnim = function(self,on)
		...
		coroutine.wrap(function()
			...
			while self.Editable do
				...
				if CurrentTick - LastTick >= 0.4 then
				...
				--print(SwitchState, AnimState, cursor.BackgroundTransparency)
	...
	funcs.Refresh = function(self)
		...
		for row = 1,maxLines do
			...
			if not lineFrame then
				...
				selectionHighlight.BackgroundColor3 = colors.SelectionBack
